...
$proj_file = Join-Path (Resolve-Path ".") "\src\XsdIterator\XsdIterator.csproj"

Get-ChildItem $ARTIFACTS | Remove-Item -Recurse -Force

& dotnet pack $proj_file /nologo -v q -c Release /p:Version=$VERSION --include-symbols --include-source --output $ARTIFACTS
if ($LASTEXITCODE -ne 0) {
